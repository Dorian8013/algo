...
CA_CONSTRAINTS="$(openssl verify -verbose \
  ...
  ./configs/10.0.8.100/ipsec/.pki/certs/google-ralgo-test-pair.com.crt 2>&1)" || true
...
ipsec up ralgovpn-10.0.8.100

ipsec statusall

ipsec statusall | grep -w ^ralgovpn-10.0.8.100 | grep -w ESTABLISHED
...
ipsec down ralgovpn-10.0.8.100
