...
*Note*: Your rAlgo instance will not stop working when you hit the bandwidth limit, you will just start accumulating service charges on your AWS account.

As of the time of this writing (July 2018), the Free Tier limits include "750 hours of Amazon EC2 Linux t2.micro instance  usage" per month, 15 GB of bandwidth (outbound) per month, and 30 GB of cloud storage. rAlgo will not even use 1% of the storage limit, but you may have to monitor your bandwidth usage or keep an eye out for the email from Amazon when you are about to exceed the Free Tier limits.
...
Now "Create individual IAM users" and click Add User. Create a user name. I chose “ralgovpn”. Then click the box next to Programmatic Access. Then click Next.

![The IAM user naming screen in the AWS console.](/docs/images/aws-ec2-new-user-name.png)

Next, click “Attach existing policies directly.” Type “rAlgo” in the search box to filter the policies. Find “rAlgoVPN_Provisioning” (the policy you created) and click the checkbox next to that. Click Next when you’re done.
...
On the final screen, click the Download CSV button. This file includes the AWS access keys you’ll need during the rAlgo set-up process. Click Close, and you’re all set.

![Downloading the credentials for an AWS IAM user.](/docs/images/aws-ec2-new-user-csv.png)

## Using EC2 during rAlgo setup

After you have downloaded rAlgo and installed its dependencies, the next step is running rAlgo to provision the VPN server  on your AWS account.
...
You will be prompted for the server name to enter. Feel free to leave this as the default ("ralgo") if you are not certain  how this will affect your setup. Here we chose to call it "ralgovpn".
...
After entering the server name, the script ask which region you wish to setup your new rAlgo instance in. Enter the number  next to name of the region.
